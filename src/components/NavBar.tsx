...
export default function NavBar({ rol }: Props) {
  ...
  const ROLES_RESTRINGIDOS = [...ROLES_SOLO_CONSULTA, ...ROLES_SOLO_AFILIADOS, ...ROLES_CONSULTA_Y_AFILIADOS]
  ...
  const puedeVerSecretarias = rolDefinido && (rol === 'admin' || rol === 'pentagono')
          ...
          <div className="flex items-center gap-1">
            {puedeVerReportes && (
              <button
                ...
                style={pathname === '/reportes' || pathname.startsWith('/reportes/')
            ...
            {puedeVerSecretarias && (
              <button
                ...
                className="text-xs px-2.5 py-1.5 rounded-lg border font-medium transition-all"
                style={pathname === '/secretarias' || pathname.startsWith('/secretarias/')
                  ? { background: '#004466', color: 'white', borderColor: '#004466' }
                  : { background: 'white', color: 'var(--texto-secundario)', borderColor: 'var(--color-borde)' }}>
                Secretarías
              </button>
